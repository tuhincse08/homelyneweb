            ...
            <div class="row align-items-center">
                <div class="col-xl-10 col-md-9 col-sm-8 mb-3 mb-sm-0 {{$parcel_order ? 'mb-2':''}}">
                    <h1 class="page-header-title text-capitalize m-0">
                        ...
                        <span>
                            @if ($parcel_order) {{translate('messages.parcel')}} {{translate('messages.orders')}} @else {{translate(str_replace('_',' ',$status))}} {{translate('messages.orders')}} @endif <span class="badge badge-soft-dark ml-2">{{$total}}</span>
                        </span>
                    </h1>
                </div>
                    ...
                    @foreach($orders as $key=>$order)
                        ...
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            <td>
                                ...
                                @elseif ($order->store)
                                    <div><a  class="text--title" href="{{route('admin.vendor.view', $order->store_id)}}" alt="view store">{{Str::limit($order->store?$order->store->name:translate('messages.store deleted!'),20,'...')}}</a></div>
                                @else
