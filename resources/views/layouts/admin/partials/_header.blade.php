            ...
            <div class="navbar-nav-wrap-content-right">
                ...
                <ul class="navbar-nav align-items-center flex-row">
                    ...
                    <li class="nav-item d-none d-sm-inline-block mr-4">
                        <!-- Notification pending stores and messege -->
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-soft-secondary rounded-circle"
                               href="{{route('admin.vendor.list')}}">
                                ...
                                @if($inactive_stores!=0)
                                    <span class="btn-status btn-sm-status btn-status-danger"></span>
                                @endif
                            </a>
                        </div>

                        <!-- Notification pending Delivery Partner -->
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-soft-secondary rounded-circle"
                               href="{{route('admin.vendor.list')}}">
                                <i class="tio-bike"></i>
                                @php($inactive_stores = \App\Models\DeliveryMan::where(['application_status'=>'pending'])->count())
                                @php($inactive_stores = isset($inactive_stores) ? $inactive_stores : 0)
                                @if($inactive_stores!=0)
                                    <span class="btn-status btn-sm-status btn-status-danger"></span>
                                @endif
