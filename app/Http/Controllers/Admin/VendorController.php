...
class VendorController extends Controller
{
        ...
        if ($request->identity_image){
            if($store->vendor->identity_image){
                foreach (json_decode($store->vendor->identity_image, true) as $img) {
                    if (Storage::disk('public')->exists('vendor/' . $img)) {
            ...
            $identity_image = $store->vendor->identity_image;
        }
    ...
    public function update_application(Request $request)
    {
        ...
        $data = [];
        $data['email'] = $store->vendor->email;
        $data['name'] = $store->vendor->f_name;
        if($request->status){
            $response = Mail::send('email-templates.shop-approved', $data, function ($mail) use($data)
                {
                
                    $mail->to($data['email'], $data['name']);
                    $mail->subject('Shop Approved');
                });}
        else{
            $response = Mail::send('email-templates.shop-denied', $data, function ($mail) use($data)
                {
                
                    $mail->to($data['email'], $data['name']);
                    $mail->subject('Shop Denied');
                });}
